...
public interface PatientRepository extends CrudRepository<Patient, Long>, PagingAndSortingRepository<Patient, Long> {
    ...
    @Query(value = "SELECT p.id AS id, p.first_name AS firstName, p.last_name AS lastName, p.birth_day as dateOfBirth, p.gender AS gender, mc.complaints as complaints FROM patients p, statements st, medical_card mc WHERE p.id=st.patient_id AND mc.statement_id =st.id AND st.patient_status='NEW' AND p.id IN (SELECT st.patient_id FROM statements st, medical_card mc, users u WHERE st.id=mc.statement_id AND mc.doctor_id=u.id AND st.patient_status='NEW' AND u.id=:doctorId)", nativeQuery = true)
...
//    List<TreatmentPatientDto> patientsForTreatment(Long doctorId);
    ...
    @Query(value = "SELECT p.id AS id, p.first_name AS firstName, p.last_name AS lastName, p.birth_day as dateOfBirth, p.gender AS gender, mc.complaints as complaints, mc.diagnosis AS diagnosis, tr.appointment AS appointment, tr.appointment_status AS appointmentStatus, u.first_name AS doctorFirstName, u.last_name AS doctorLastName, c.title AS doctorCategory FROM patients p, statements st, medical_card mc, treatment tr, users u, categories c WHERE c.id=u.category_id AND u.id=mc.doctor_id AND p.id=st.patient_id AND tr.id=mc.treatment_id AND mc.statement_id =st.id AND p.id NOT IN (SELECT st.patient_id FROM statements st WHERE st.patient_status='DISCHARGED') AND p.id IN (SELECT st.patient_id FROM statements st, medical_card mc, users u, treatment tr WHERE tr.id=mc.treatment_id AND st.id=mc.statement_id AND mc.doctor_id=u.id AND tr.appointment_status='NOT_EXECUTED' AND st.patient_status='DIAGNOSED' AND u.id=:doctorId)", nativeQuery = true)
    List<TreatmentPatientDto> patientsForTreatment(Long doctorId);

    @Query(value = "SELECT p.id AS id, p.first_name AS firstName, p.last_name AS lastName, p.birth_day as dateOfBirth, p.gender AS gender, mc.complaints as complaints, mc.diagnosis AS diagnosis, tr.appointment AS appointment, tr.appointment_status AS appointmentStatus, u.first_name AS doctorFirstName, u.last_name AS doctorLastName, c.title AS doctorCategory, tr.id AS treatmentId, st.id AS statementId FROM patients p, statements st, medical_card mc, treatment tr, users u, categories c WHERE c.id=u.category_id AND u.id=mc.doctor_id AND p.id=st.patient_id AND tr.id=mc.treatment_id AND mc.statement_id =st.id AND p.id IN (SELECT st.patient_id FROM statements st, medical_card mc, users u, treatment tr, patients p WHERE p.id=st.patient_id AND tr.id=mc.treatment_id AND st.id=mc.statement_id AND mc.doctor_id=u.id AND tr.appointment_status='NOT_EXECUTED' AND st.patient_status='DIAGNOSED' AND u.id=:doctorId AND p.id=:patientId)", nativeQuery = true)
    TreatmentPatientDto patientForTreatment(Long doctorId, Long patientId);

    @Query(value = "SELECT p.id AS id, p.first_name AS firstName, p.last_name AS lastName, p.birth_day as dateOfBirth, p.gender AS gender, mc.complaints as complaints, mc.diagnosis AS diagnosis, tr.appointment AS appointment, tr.appointment_status AS appointmentStatus, u.first_name AS doctorFirstName, u.last_name AS doctorLastName, c.title AS doctorCategory FROM patients p, statements st, medical_card mc, treatment tr, users u, categories c WHERE c.id=u.category_id AND u.id=mc.doctor_id AND p.id=st.patient_id AND tr.id=mc.treatment_id AND mc.statement_id =st.id AND p.id IN (SELECT st.patient_id FROM statements st, medical_card mc, users u, treatment tr WHERE tr.id=mc.treatment_id AND st.id=mc.statement_id AND mc.doctor_id=u.id AND tr.appointment_status='EXECUTED' AND st.patient_status='DISCHARGED' AND u.id=:doctorId)", nativeQuery = true)
